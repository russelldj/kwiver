...
// Number of bins for the internal color histogram used to estimate mask
// properties
    ...
    cumulative_mask_count_( 0.0 )
{
  if( !this->configure( options ) )
  {
    ...
    VIDTK_ERROR( "Unable to configure scene_obstructor_detector!" );
  }
}
...
::configure( const scene_obstruction_detector_settings& options )
{
  ...
  if( !options_.is_training_mode_ )
  {
    ...
    {
      if( !appearance_classifier_.load_from_file( options.
                                                  appearance_classifier_filename_ ) )
      {
        VIDTK_ERROR( "Invalid appearance classifier specified" );
        return false;
      }
    }
  }
  ...
  // Compute bitshift based on the input type which maps each pixel to a 16
  // binned
  ...
  unsigned scale_factor =
    ( static_cast< unsigned >( std::numeric_limits< PixType >::max() ) + 1 ) /
    hist_bins;
  ...
  if( options.is_training_mode_ )
  {
    ...
    trainer_options.gt_loader.load_mode =
      pixel_annotation_loader_settings::DIRECTORY;
    trainer_options.gt_loader.path = options.groundtruth_dir_;

    if( !training_data_extractor_->configure( trainer_options ) )
    {
      VIDTK_ERROR( "Error setting up training data extractor" );
      return false;
    }
  }

  if( options.use_spatial_prior_feature_ &&
      !options.spatial_prior_filename_.empty() )
  {
    ...
    {
      VIDTK_ERROR(
        "Unable to load spatial prior image: " <<
          options.spatial_prior_filename_ );
      return false;
    }

    if( spatial_prior_image_.nplanes() != 1 )
    {
      VIDTK_ERROR( "Spatial prior image must contain 1 plane!" );
      return false;
    }
  }
...
::configure_spatial_prior( const source_image& input )
{
  if( spatial_prior_image_.size() == 0 )
  {
    ...
    for( unsigned i = 0; i < input.ni(); i++ )
    {
      ...
      {
        vxl_byte i_id = ( options_.spatial_prior_grid_length_ * i ) /
                        input.ni();
        vxl_byte j_id = ( options_.spatial_prior_grid_length_ * j ) /
                        input.nj();
        spatial_prior_image_( i,
                              j ) = options_.spatial_prior_grid_length_ *
                                    j_id + i_id;
      }
    }
  }
  ...
  {
    vil_image_view< FeatureType > new_spatial_prior_image( input.ni(),
                                                           input.nj(), 1 );
    vil_resample_bilin( spatial_prior_image_, new_spatial_prior_image,
                        input.ni(), input.nj() );
                                 ...
                                 classified_image& output_image )
{
  ...
  {
    appearance_classifier_.classify_images( input_array, approx_output,
                                            offset );
  }
                            ...
                            const classified_image& output )
{
  ...
  VIDTK_ASSERT(
    input.nplanes() == 3 || input.nplanes() == 1, "Invalid plane count" );
        ...
        options_.intensity_diff_req_ ) )
  {
    ...
    mask_count_history_.insert( pix_count );
    mask_intensity_history_.insert( props_.intensity_ );
    VIDTK_INFO( "HUD change detected on frame: " << frame_counter_ );
    return true;
  }
...
{
  std::string frame_id = std::to_string( frame_counter_ - 1 );
...
// ----------------------------------------------------------------------------
// Debug function used for outputing a single classified image, scaled to value
// max_to_rep
void
                               ...
                               double max_to_rep )
{
  ...
  vil_image_view< vxl_byte > writeme( classifier_image.ni(),
                                      classifier_image.nj(), 3 );
