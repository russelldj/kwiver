...
::configure( const scene_obstruction_detector_settings& options )
{
  ...
  if( options.use_spatial_prior_feature_ && !options.spatial_prior_filename_.empty() )
  {
    ...
    {
      VIDTK_ERROR( "Unable to load spatial prior image: " << options.spatial_prior_filename_ );
      return false;
    }
                 ...
                 properties& output_properties )
{
  ...
  VIDTK_ASSERT( input_features[0].ni() == input_image.ni(), "Input widths do not match." );
  VIDTK_ASSERT( input_features[0].nj() == input_image.nj(), "Input heights do not match." );
                            ...
                            const classified_image& output )
{
  ...
  VIDTK_ASSERT( input.nplanes() == 3 || input.nplanes() == 1, "Invalid plane count" );
