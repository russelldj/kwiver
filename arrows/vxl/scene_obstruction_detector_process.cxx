...
::set_params( config_block const& blk )
{
  ...
  try
  {
    ...
    options_.output_feature_image_mode_ = blk.get< bool >(
                                            "output_feature_images" );
    options_.output_classifier_image_mode_ = blk.get< bool >(
                                               "output_classifier_images" );
    options_.use_spatial_prior_feature_ = blk.get< bool >(
                                            "use_spatial_prior_feature" );

    if( options_.use_spatial_prior_feature_ )
    {
      options_.spatial_prior_filename_ = blk.get< std::string >(
                                           "spatial_prior_filename" );
    }
    ...
    if( !options_.is_training_mode_ )
    {
      ...
      options_.use_appearance_classifier_ = blk.get< bool >(
                                              "use_appearance_filter" );
      options_.appearance_frames_ = blk.get< unsigned >(
                                      "appearance_frame_count" );
      options_.primary_classifier_filename_ = blk.get< std::string >(
                                                "intial_classifier" );

      if( options_.use_appearance_classifier_ )
      {
        options_.appearance_classifier_filename_ = blk.get< std::string >(
                                                     "appearance_classifier" );
      }
    }

    // Load color-related parameters
    options_.map_colors_to_nearest_extreme_ = blk.get< bool >(
                                                "map_colors_to_nearest_extreme" );
    options_.map_colors_near_extremes_only_ = blk.get< bool >(
                                                "map_colors_near_extremes_only" );

    // Load adaptive thresholding parameters
    options_.use_adaptive_thresh_ = blk.get< bool >(
    ...
    if( options_.use_adaptive_thresh_ )
    {
      ...
      options_.at_interval_1_adj_ =
        blk.get< double >( "interval_1_adjustment" );
      options_.at_interval_2_adj_ =
        blk.get< double >( "interval_2_adjustment" );
      options_.at_interval_3_adj_ =
        blk.get< double >( "interval_3_adjustment" );
    }

    // Load break detection parameters
    options_.enable_mask_break_detection_ = blk.get< bool >(
                                              "enable_mask_break_detection" );
    if( options_.enable_mask_break_detection_ )
    {
      options_.mask_count_history_length_ = blk.get< unsigned >(
                                              "mask_count_history_length" );
      options_.mask_intensity_history_length_ = blk.get< unsigned >(
                                                  "mask_intensity_history_length" );
      options_.count_percent_change_req_ = blk.get< double >(
                                             "count_percent_change_req" );
      options_.count_std_dev_req_ = blk.get< double >( "count_std_dev_req" );
      options_.min_hist_for_intensity_diff_ = blk.get< unsigned >(
                                                "min_hist_for_intensity_diff" );
  ...
  {
    VIDTK_ERROR( this->name() << ": set_params failed: " << e.what() );
    return false;
  }
...
::step2()
{
  ...
  if( !validate_inputs() )
  {
    VIDTK_ERROR( this->name() << ": Invalid Inputs!" );
